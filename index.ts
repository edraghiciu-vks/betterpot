// Beatport API Test Client - No Python Required!
// This replicates the beets-beatport4 approach in pure TypeScript
...
async function main() {
  console.log('🎵 Betterpot - Beatport API Client (No Python Required!)\n');
  ...
  const clientId = process.env.BEATPORT_CLIENT_ID; // Optional

  const api = new BeatportAPI(username, password, clientId);
  ...
  try {
    ...
    if (hasValidToken) {
      ...
      console.log('⚡ No valid stored token, authenticating...');
      
      // Method 1: Try username/password authentication
      if (username && password) {
        console.log('🔑 Attempting username/password authentication...');
        const tokenData = await api.authenticateWithPassword();
        console.log(`✅ Authentication successful!`);
        console.log(`Token type: ${tokenData.token_type}`);
        console.log(`Expires in: ${tokenData.expires_in} seconds\n`);
      } else {
        // Method 2: Manual token method
        console.log('❌ No username/password found in environment variables.');
        console.log('\nTrying manual token method...\n');
        
        try {
          await api.authenticateWithManualToken();
        } catch (manualError) {
          console.log('\n📝 To get started, create a .env file with:');
          console.log('BEATPORT_USERNAME=your_beatport_username');
          console.log('BEATPORT_PASSWORD=your_beatport_password');
          console.log('\nOR for manual token method:');
          console.log('BEATPORT_TOKEN={"access_token":"your_token_here","expires_in":36000,"token_type":"Bearer"}');
          console.log('\n💡 This approach uses the same public client_id as the beets-beatport4 plugin!');
          return;
        }
      }
    }
    ...
    const introspectionData = await api.introspect();
    console.log('✅ Token valid!', introspectionData);

    // Example API calls
    console.log('\n🎯 Example API calls:');
    
    try {
      const trackResults = await api.searchTracks('deadmau5') as any;
      console.log(`Found ${trackResults.tracks?.length || 0} tracks for "deadmau5"`);
      
      const releaseResults = await api.searchReleases('random album names') as any;
      console.log(`Found ${releaseResults.releases?.length || 0} releases for "random album names"`);
    } catch (apiError) {
      console.log('⚠️ API search failed:', apiError);
    }
